#!/usr/bin/env python3
# compare_diff_gui.py
# GUI 工具：比较两个表格，提取只在 A 中出现不在 B 中出现的行
...
def run_compare():
    file_a = entry_a.get()
    file_b = entry_b.get()
    key_col = entry_key.get().strip()
    output = entry_out.get()
    if not all([file_a, file_b, key_col, output]):
    ...
    if key_col not in df_a.columns or key_col not in df_b.columns:
        messagebox.showerror("列名错误", f"列 “{key_col}” 在表 A 或表 B 中不存在。")
        return

    # 去重
    df_a = df_a.drop_duplicates(subset=[key_col], keep='first')
    df_b = df_b.drop_duplicates(subset=[key_col], keep='first')

    # 差集
    mask = ~df_a[key_col].isin(df_b[key_col])
    df_diff = df_a[mask]
...
# 键 列
tk.Label(root, text="对比列名：").grid(row=2, column=0, sticky="e", padx=padx, pady=pady)
entry_key = tk.Entry(root, width=20)
entry_key.grid(row=2, column=1, sticky="w", padx=padx, pady=pady)

# 输出
tk.Label(root, text="输出文件：").grid(row=3, column=0, sticky="e", padx=padx, pady=pady)
entry_out = tk.Entry(root, width=50)
entry_out.grid(row=3, column=1, padx=padx, pady=pady)
tk.Button(root, text="保存为…", command=lambda: entry_out.insert(0, filedialog.asksaveasfilename(defaultextension=".xlsx", filetypes=[("Excel","*.xlsx;*.xls"),("CSV","*.csv")]))).grid(row=3, column=2, padx=padx)

# 运行按钮
tk.Button(root, text="运行", width=12, command=run_compare).grid(row=4, column=1, pady=20)
